...
public interface SalesDao extends PagingAndSortingRepository<Sales, String> {
	...
	@Query("select new map (m.id as id,m.nama as nama,m.grupId as grupId,n.id as userId,n.nama as userNama, m.isActive as isActive, "
			+ "m.version as version, m.createBy as createBy, m.createDate as createDate) "
			+ "from Sales m left join m.user n ")
